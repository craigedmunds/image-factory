...
def _generate_build_workflow(image: dict, output_dir: Path) -> bool:
    ...
    patch_file = source.get("patchFile")
    ...
    # When patchFile is set, add steps to checkout the state repo
    # and apply the patch to the upstream source before building.
    # The patch will fail loudly if upstream changes the patched lines.
    if patch_file:
        patch_steps = f"""
      - name: Checkout patch
        ...
        with:
          path: _image-factory
          sparse-checkout: {patch_file}
          sparse-checkout-cone-mode: false

      - name: Apply patch
        run: git apply _image-factory/{patch_file}
"""
    else:
        patch_steps = ""
