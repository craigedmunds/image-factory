...
def _generate_build_workflow(image: dict, output_dir: Path) -> bool:
    ...
    # Build local checkout and patch steps if needed
    has_local_files = patch_files or version_file
    local_steps = ""

    if has_local_files:
        # Checkout the state repo (sparse, just the local dir)
...
"""
    ...
    for patch in patch_files:
        # Resolve patch path: if it's just a filename, prefix with local_dir
...
"""
    ...
    if version_path:
        # Read VERSION from file; workflow_dispatch input overrides if provided
        tag_input_default = ""
        tag_expr = f"""${{{{ inputs.tag || steps.version.outputs.version }}}}"""
        local_steps += f"""
      - name: Read version
        id: version
...
"""
    ...
    # Check if workflow already exists and is unchanged
...
on:
  workflow_dispatch:
    inputs:
      tag:
        description: "Image tag to build (leave empty to use VERSION file)"
        required: false{tag_input_default}
        type: string
