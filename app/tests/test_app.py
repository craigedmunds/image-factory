...
class TestGenerateBuildWorkflow:
    ...
    def test_generates_standard_workflow(self, output_dir):
        """Test generating a standard workflow without localDockerfile."""
        ...
        # Should NOT have local Dockerfile steps
        assert "Checkout local Dockerfile" not in content
        assert "Use local Dockerfile" not in content

    def test_generates_workflow_with_patch_file(self, output_dir):
        """Test generating a workflow with patchFile applied to upstream source."""
        from generate_state import _generate_build_workflow
        ...
        content = self._read_workflow(output_dir, "test-app")
        assert "# Auto-generated by Image Factory" in content
        assert "Checkout patch" in content
        assert "sparse-checkout: patches/test-app/Dockerfile.patch" in content
